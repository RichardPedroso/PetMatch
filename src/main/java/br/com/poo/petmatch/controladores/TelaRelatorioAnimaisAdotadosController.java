...
public class TelaRelatorioAnimaisAdotadosController {

    public static final String FXML_PATH = "tela-relatorio-animais-adotados.fxml";
    ...
    private void initialize() {
        ...
        labelAnimalFicticio.setVisible(false);
        mostrarAnimal();
        ...
        botaoVoltarTela.setOnMouseClicked(event -> {
            try{
                voltarParaTelaOpcoesAdministrador();
            }catch (Exception e){
                e.printStackTrace();
            }
        });
    ...
    private void mostrarAnimal(){
        animalAtual = PetMatch.controlePetMatch.relatorioAnimaisAdotados.get(indiceAtual);
    ...
    private void proximoAnimal(){
        if(indiceAtual < PetMatch.controlePetMatch.relatorioAnimaisAdotados.size()-1) {
            indiceAtual++;
            if (indiceAtual < PetMatch.controlePetMatch.relatorioAnimaisAdotados.size()) {
                animalAtual = PetMatch.controlePetMatch.relatorioAnimaisAdotados.get(indiceAtual);
                atualizarAnimal(animalAtual);
            }
        }else{
    ...
    private void voltarParaTelaOpcoesAdministrador() throws Exception{
